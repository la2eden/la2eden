 ...
 */
package com.la2eden.gameserver.model.quest;
...
import com.la2eden.gameserver.enums.TrapAction;
...
import com.la2eden.gameserver.model.olympiad.CompetitionType;
import com.la2eden.gameserver.model.olympiad.Participant;
...
import com.la2eden.gameserver.network.serverpackets.NpcQuestHtmlMessage;
...
public class Quest extends AbstractScript implements IIdentifiable
{
	 ...
	 * <b>Note 3:</b><br>
	 * The returned value of onFirstTalk replaces the default HTML that would have otherwise been loaded from a sub-folder of gameserver/datapack/html/.<br>
	...
	{
		final String result = HtmCache.getInstance().getHtm(player.getHtmlPrefix(), "./datapack/html/noquest.htm");
	...
	{
		final String result = HtmCache.getInstance().getHtm(player.getHtmlPrefix(), "./datapack/html/alreadycompleted.htm");
	...
	{
		return (target == null) || com.la2eden.gameserver.util.Util.checkIfInRange(1500, player, target, true);
	}
	...
	public String getHtm(String prefix, String fileName)
	{
		...
		String content = hc.getHtm(prefix, fileName.startsWith("./datapack/") ? fileName : "./datapack/scripts/" + getDescr().toLowerCase() + "/" + getName() + "/" + fileName);
		if (content == null)
		{
			content = hc.getHtm(prefix, "./datapack/scripts/" + getDescr() + "/" + getName() + "/" + fileName);
			if (content == null)
			{
				content = hc.getHtmForce(prefix, "./datapack/scripts/quests/" + getName() + "/" + fileName);
			}
		}
		return content;
	}

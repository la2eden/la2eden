...
public class L2PetManagerInstance extends L2MerchantInstance
{
	...
	public String getHtmlPath(int npcId, int val)
	{
		...
		return "./datapack/html/petmanager/" + pom + ".htm";
	}
	
	@Override
	public void showChatWindow(L2PcInstance player)
	{
		String filename = "./datapack/html/petmanager/" + getId() + ".htm";
		if ((getId() == 36478) && player.hasSummon())
		{
			filename = "./datapack/html/petmanager/restore-unsummonpet.htm";
		}
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (command.startsWith("evolve"))
		{
			...
			{
				final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile(player.getHtmlPrefix(), "./datapack/html/petmanager/evolve_no.htm");
				player.sendPacket(html);
			}
			return;
		}
		else if (command.startsWith("restore"))
		{
			...
			{
				final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile(player.getHtmlPrefix(), "./datapack/html/petmanager/restore_no.htm");
				player.sendPacket(html);
			}
			return;
		}
	...
	public final void exchange(L2PcInstance player, int itemIdtake, int itemIdgive)
	{
		...
		if (player.destroyItemByItemId("Consume", itemIdtake, 1, this, true))
		{
			...
			html.setFile(player.getHtmlPrefix(), "./datapack/html/petmanager/" + getId() + ".htm");
			player.sendPacket(html);
		}
		else
		{
			html.setFile(player.getHtmlPrefix(), "./datapack/html/petmanager/exchange_no.htm");
			player.sendPacket(html);
		}
	}
}

...
public class L2DoormenInstance extends L2NpcInstance
{
	...
	public void showChatWindow(L2PcInstance player)
	{
		...
		if (!isOwnerClan(player))
		{
			html.setFile(player.getHtmlPrefix(), "./datapack/html/doormen/" + getTemplate().getId() + "-no.htm");
		}
		else
		{
			html.setFile(player.getHtmlPrefix(), "./datapack/html/doormen/" + getTemplate().getId() + ".htm");
		}
	...
	protected void cannotManageDoors(L2PcInstance player)
	{
		...
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		html.setFile(player.getHtmlPrefix(), "./datapack/html/doormen/" + getTemplate().getId() + "-busy.htm");
		player.sendPacket(html);
	}

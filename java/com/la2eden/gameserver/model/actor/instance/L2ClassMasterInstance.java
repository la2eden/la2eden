...
import com.la2eden.gameserver.network.serverpackets.TutorialShowHtml;
import com.la2eden.gameserver.network.serverpackets.TutorialShowQuestionMark;
...
public final class L2ClassMasterInstance extends L2MerchantInstance
{
	...
	public String getHtmlPath(int npcId, int val)
	{
		...
		return "./datapack/html/classmaster/" + pom + ".htm";
	}
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (command.startsWith("change_class"))
		{
			...
			if (checkAndChangeClass(player, val))
			{
				...
				html.setFile(player.getHtmlPrefix(), "./datapack/html/classmaster/ok.htm");
		...
		else if (command.startsWith("become_noble"))
		{
			if (!player.isNoble())
			{
				...
				final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile(player.getHtmlPrefix(), "./datapack/html/classmaster/nobleok.htm");
				player.sendPacket(html);
			}
		}
		...
		else if (command.startsWith("increase_clan_level"))
		{
			...
			{
				final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile(player.getHtmlPrefix(), "./datapack/html/classmaster/noclanleader.htm");
				player.sendPacket(html);
			}
			else if (player.getClan().getLevel() >= 5)
			{
				final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile(player.getHtmlPrefix(), "./datapack/html/classmaster/noclanlevel.htm");
				player.sendPacket(html);
			}
			else
			{
	...
	private static final void showHtmlMenu(L2PcInstance player, int objectId, int level)
	{
		...
		{
			html.setFile(player.getHtmlPrefix(), "./datapack/html/classmaster/disabled.htm");
		}
		...
		else
		{
			...
			{
				html.setFile(player.getHtmlPrefix(), "./datapack/html/classmaster/nomore.htm");
			}
			else
			{
				...
				if ((player.getLevel() >= minLevel) || Config.ALLOW_ENTIRE_TREE)
				{
					...
					if (menu.length() > 0)
					{
						html.setFile(player.getHtmlPrefix(), "./datapack/html/classmaster/template.htm");
						html.replace("%name%", ClassListData.getInstance().getClass(currentClassId).getClientCode());
						html.replace("%menu%", menu.toString());
					}
					else
					{
						html.setFile(player.getHtmlPrefix(), "./datapack/html/classmaster/comebacklater.htm");
	...
	private static final void showTutorialHtml(L2PcInstance player)
	{
		...
		String msg = HtmCache.getInstance().getHtm(player.getHtmlPrefix(), "./datapack/html/classmaster/tutorialtemplate.htm");

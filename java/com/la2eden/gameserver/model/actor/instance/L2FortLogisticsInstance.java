...
public class L2FortLogisticsInstance extends L2MerchantInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		if (actualCommand.equalsIgnoreCase("rewards"))
		{
			if (isMyLord(player))
			{
				html.setFile(player.getHtmlPrefix(), "./datapack/html/fortress/logistics-rewards.htm");
				html.replace("%bloodoath%", String.valueOf(player.getClan().getBloodOathCount()));
			}
			else
			{
				html.setFile(player.getHtmlPrefix(), "./datapack/html/fortress/logistics-noprivs.htm");
			}
		...
		else if (actualCommand.equalsIgnoreCase("blood"))
		{
			if (isMyLord(player))
			{
				...
				if (blood > 0)
				{
					...
					html.setFile(player.getHtmlPrefix(), "./datapack/html/fortress/logistics-blood.htm");
				}
				else
				{
					html.setFile(player.getHtmlPrefix(), "./datapack/html/fortress/logistics-noblood.htm");
				}
			}
			else
			{
		...
		else if (actualCommand.equalsIgnoreCase("supplylvl"))
		{
			if (getFort().getFortState() == 2)
			{
				...
				{
					html.setFile(player.getHtmlPrefix(), "./datapack/html/fortress/logistics-supplylvl.htm");
			...
			{
				html.setFile(player.getHtmlPrefix(), "./datapack/html/fortress/logistics-1.htm"); // TODO: Missing HTML?
			}
			html.replace("%objectId%", String.valueOf(getObjectId()));
			player.sendPacket(html);
		}
		else if (actualCommand.equalsIgnoreCase("supply"))
		{
			if (isMyLord(player))
			{
				...
				{
					html.setFile(player.getHtmlPrefix(), "./datapack/html/fortress/logistics-siege.htm");
				}
				else
				{
					...
					if (level > 0)
					{
						...
						html.setFile(player.getHtmlPrefix(), "./datapack/html/fortress/logistics-supply.htm");
					}
					else
					{
						html.setFile(player.getHtmlPrefix(), "./datapack/html/fortress/logistics-nosupply.htm");
					}
				}
			}
			else
	...
	private void showMessageWindow(L2PcInstance player, int val)
	{
		...
		if (val == 0)
		{
			filename = "./datapack/html/fortress/logistics.htm";
		}
		else
		{
			filename = "./datapack/html/fortress/logistics-" + val + ".htm";
		}
	...
	public String getHtmlPath(int npcId, int val)
	{
		...
		return "./datapack/html/fortress/" + pom + ".htm";
	}

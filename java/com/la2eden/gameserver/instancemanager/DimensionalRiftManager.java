...
import com.la2eden.gameserver.model.DimensionalRiftRoom;
...
public final class DimensionalRiftManager
{
	...
	public void loadSpawns()
	{
		...
		try
		{
			...
			final File file = new File(Config.DATAPACK_ROOT, "./datapack/DimensionalRift.xml");
			if (!file.exists())
			{
	...
	public synchronized void start(L2PcInstance player, byte type, L2Npc npc)
	{
		...
		if (!player.isInParty())
		{
			showHtmlFile(player, "./datapack/html/seven_signs/rift/NoParty.htm", npc);
			return;
		}
		
		if (player.getParty().getLeaderObjectId() != player.getObjectId())
		{
			showHtmlFile(player, "./datapack/html/seven_signs/rift/NotPartyLeader.htm", npc);
			return;
		}
		...
		if (player.getParty().getMemberCount() < Config.RIFT_MIN_PARTY_SIZE)
		{
			...
			html.setFile(player.getHtmlPrefix(), "./datapack/html/seven_signs/rift/SmallParty.htm");
		...
		if (!canPass)
		{
			showHtmlFile(player, "./datapack/html/seven_signs/rift/NotInWaitingRoom.htm", npc);
			return;
		}
		...
		{
			final NpcHtmlMessage html = new NpcHtmlMessage(npc.getObjectId());
			html.setFile(player.getHtmlPrefix(), "./datapack/html/seven_signs/rift/NoFragments.htm");
			html.replace("%npc_name%", npc.getName());
	...
	{
		showHtmlFile(player, "./datapack/html/seven_signs/rift/Cheater.htm", npc);
		if (!player.isGM())
		{
